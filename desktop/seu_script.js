...
async function loadArticles() {
    ...
    let searchFieldFirestore = searchTypeInput.value; // Pega o valor selecionado (ex: 'title', 'description')

    // Mapeia o campo de busca selecionado para o campo em minúsculas no Firestore
    // Isso garante que a busca seja case-insensitive
    if (searchFieldFirestore === 'title') {
        searchFieldFirestore = 'title_lower';
    } else if (searchFieldFirestore === 'description') {
        searchFieldFirestore = 'description_lower';
    }
    // Repita para 'summary', 'abstract', 'introduction', etc., se você os armazenar em _lower

    console.log("Termo de busca (minúsculas):", searchTerm);
    console.log("Campo de busca no Firestore:", searchFieldFirestore); // Verifique este log!

    try {
        let articlesRef = firestore.collection('articlesdoc');

        if (searchTerm && searchFieldFirestore) { // Use searchFieldFirestore aqui
            articlesRef = articlesRef
                .where(searchFieldFirestore, '>=', searchTerm)
                .where(searchFieldFirestore, '<=', searchTerm + '\uf8ff');
        }
        // ... (restante da lógica de filtros e ordenação) ...
        if (searchTerm && searchFieldFirestore) { // Use searchFieldFirestore aqui
            articlesRef = articlesRef.orderBy(searchFieldFirestore);
        } else {
